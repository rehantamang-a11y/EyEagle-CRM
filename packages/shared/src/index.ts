...
export const websiteIntakeSchema = z.object({
  ...
  consideringFor: z.array(z.string().trim().max(160)).max(10).optional(),
  safetyConcerns: z.array(z.string().trim().max(160)).max(12).optional(),
  immediateSafetyConcern: z.boolean().optional(),
  expressedInterest: z.string().trim().max(200).optional(),
  /** @deprecated Accepted while older Jotform field maps are migrated. */
  requestedNextStep: z.string().trim().max(200).optional(),
  preferredContactDay: z.string().trim().max(80).optional(),
  preferredContactPeriod: z.string().trim().max(80).optional(),
