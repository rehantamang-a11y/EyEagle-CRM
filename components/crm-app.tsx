...
export function CRMApp() {
  ...
  const myWorkQuery = useMyWorkOpportunities(view === "work");
  const syncJotform = useSyncJotform();
  const takeOwnership = useTakeOwnership();
  const saveOpportunityAction = useSaveOpportunityAction();
  const hasBackendOpportunityId = Boolean(detail?.id && !detail.id.startsWith("demo-"));
  const showActivityHistory = Boolean(detail && (view === "all" || (view === "work" && detail.workGroup === "CLOSED")));
  const opportunityDetailQuery = useOpportunityDetails(detail?.id, Boolean(detail) && view !== "new" && hasBackendOpportunityId);
  const actionHistoryQuery = useOpportunityActionHistory(detail?.id, showActivityHistory && hasBackendOpportunityId);
  ...
  const visibleItems = view === "new"
    ...
    : view === "work"
      ? (myWorkQuery.data || [])
      : items;
  const rows = useMemo(() => visibleItems.filter((item) => {
    ...
    if (view === "work" && item.workGroup) {
      return item.workGroup === (filter === "follow_ups" ? "FOLLOW_UPS" : filter.toUpperCase());
    }
    if (filter === "closed") return item.status === "won" || item.status === "lost";
    if (item.status !== "open") return false;
    if (!item.nextActionAt) return filter === "due";
    return filter === "due" ? new Date(item.nextActionAt) <= new Date() : new Date(item.nextActionAt) > new Date();
  }).sort((a, b) => view === "new" ? new Date(a.submittedAt).getTime() - new Date(b.submittedAt).getTime() : new Date(a.nextActionAt || a.closedAt || 0).getTime() - new Date(b.nextActionAt || b.closedAt || 0).getTime()), [visibleItems, view, filter, search]);
  const newCount = unclaimedQuery.data?.length || 0;
  const openCount = myWorkQuery.data?.filter((item) => item.status === "open").length || 0;
  const activeListQuery = view === "new" ? unclaimedQuery : view === "work" ? myWorkQuery : null;
