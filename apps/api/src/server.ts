...
app.post("/api/v1/leads/:id/claim", async (request, reply) => {
  ...
  const result = await transaction(async (client) => {
    ...
    const updated = await client.query(`update leads set owner_user_id=$1,status='active',claimed_at=now(),first_action_due_at=now()+interval '2 hours',updated_at=now(),version=version+1,stage_id=(select id from pipeline_stages where name='Picked Up') where id=$2 and status='unclaimed' and owner_user_id is null returning id`, [actor.id, id]);
...
app.post("/api/v1/intake/website", { config: { rawBody: true } }, async (request, reply) => {
  ...
  const result=await transaction(async(client)=>{const prior=await client.query("select id,result from website_intake_submissions where idempotency_key=$1",[idempotencyKey]);if(prior.rows[0])return {intakeId:prior.rows[0].id,result:prior.rows[0].result,replayed:true};const existing=await client.query<{customer_id:string;lead_id:string;owner_user_id:string}>(`select c.id customer_id,l.id lead_id,l.owner_user_id from customers c left join leads l on l.customer_id=c.id and l.status in ('unclaimed','active') where c.normalized_phone=$1 order by l.created_at desc nulls last limit 1`,[phone]);let customerId=existing.rows[0]?.customer_id;let leadId=existing.rows[0]?.lead_id;let outcome="existing_open_lead";if(leadId){await client.query(`insert into audit_events(action,entity_type,entity_id,metadata) values('website.enquiry_received','lead',$1,$2)`,[leadId,JSON.stringify(payload)]);if(existing.rows[0].owner_user_id)await client.query(`insert into notifications(user_id,type,title,body,entity_type,entity_id) values($1,'repeat_enquiry','New enquiry from an existing lead',$2,'lead',$3)`,[existing.rows[0].owner_user_id,payload.summary,leadId]);}else{if(!customerId){const customer=await client.query<{id:string}>("insert into customers(full_name,primary_phone,normalized_phone,email,city) values($1,$2,$3,$4,$5) returning id",[payload.fullName,payload.phone,phone,payload.email||null,payload.city||null]);customerId=customer.rows[0].id;outcome="created_customer_and_lead";}else outcome="created_lead_for_customer";const lead=await client.query<{id:string}>(`insert into leads(customer_id,stage_id,source_id,priority,enquiry_summary,status) select $1,s.id,src.id,$2,$3,'unclaimed' from pipeline_stages s,lead_sources src where s.name='New Enquiry' and src.name='Website' returning id`,[customerId,payload.priority,payload.summary]);leadId=lead.rows[0].id;}const intake=await client.query<{id:string}>("insert into website_intake_submissions(idempotency_key,payload,result,customer_id,lead_id) values($1,$2,$3,$4,$5) returning id",[idempotencyKey,JSON.stringify(payload),outcome,customerId,leadId]);return {intakeId:intake.rows[0].id,result:outcome,replayed:false};});return reply.code(result.replayed?200:202).send(result);
});

/*
 * "Refresh Jotform" (system-guide.md §1): a team member pulls new submissions
 * on demand. Each submission is independent — one held-for-review row never
 * blocks the rest of the batch. Immediate concern only ever raises priority;
 * it creates no task, appointment, or emergency workflow. A preferred callback
 * day/time is folded into the summary as context only, never written to
 * next_activity_at or turned into an activity.
 */
app.post("/api/v1/intake/jotform/refresh", async (request, reply) => {
  const actor = requireAdmin(request, reply); if (!actor) return;
  const apiKey = process.env.JOTFORM_API_KEY; const formId = process.env.JOTFORM_FORM_ID;
  if (!apiKey || !formId) return reply.code(501).send({ error: { code: "JOTFORM_NOT_CONFIGURED", message: "Set JOTFORM_API_KEY and JOTFORM_FORM_ID to enable this." } });

  let submissions: Awaited<ReturnType<typeof fetchJotformSubmissions>>;
  try {
    submissions = await fetchJotformSubmissions({ apiKey, formId });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await query(`insert into jotform_sync_state(form_id,last_synced_at,last_synced_by,last_run_error) values($1,now(),$2,$3)
      on conflict(form_id) do update set last_synced_at=excluded.last_synced_at,last_synced_by=excluded.last_synced_by,last_run_error=excluded.last_run_error`,
      [formId, actor.id, message.slice(0, 1000)]);
    return reply.code(502).send({ error: { code: "JOTFORM_FETCH_FAILED", message: "Could not reach Jotform.", detail: message } });
  }

  let created = 0, skipped = 0, held = 0; let latestSubmittedAt: string | null = null;
  for (const submission of submissions) {
    latestSubmittedAt = submission.created_at;
    const already = await query("select 1 from jotform_submissions where jotform_submission_id=$1", [submission.id]);
    if (already.rowCount) { skipped++; continue; }

    const mapped = mapJotformSubmission(submission);
    if (!mapped) {
      held++;
      await query(`insert into jotform_submissions(jotform_submission_id,submitted_at,payload,status,unmapped_fields) values($1,$2,$3,'held_for_review',$4) on conflict do nothing`,
    ...
    const outcome = await transaction(async (client) => {
      const existing = await client.query<{ customer_id: string; lead_id: string | null; owner_user_id: string | null; do_not_contact: boolean }>(
        `select c.id as customer_id, l.id as lead_id, l.owner_user_id, c.do_not_contact
           from customers c left join leads l on l.customer_id=c.id and l.status in ('unclaimed','active')
          where c.normalized_phone=$1 order by l.created_at desc nulls last limit 1`, [phone]);
      let customerId = existing.rows[0]?.customer_id; let leadId = existing.rows[0]?.lead_id ?? null; let status = "existing_open_lead";

      if (leadId) {
        // Duplicate submissions on an open lead preserve whatever the salesperson
        // is already doing — this only adds a timeline event, never touches
        // stage, owner, or next_activity_at.
        await client.query(`insert into audit_events(action,entity_type,entity_id,metadata) values('jotform.enquiry_received','lead',$1,$2)`,
          [leadId, JSON.stringify({ submissionId: submission.id, priority: mapped.priority })]);
        if (existing.rows[0].owner_user_id) {
          await client.query(`insert into notifications(user_id,type,title,body,entity_type,entity_id) values($1,'repeat_enquiry','New enquiry from an existing lead',$2,'lead',$3)`,
            [existing.rows[0].owner_user_id, mapped.summary, leadId]);
        }
      } else if (existing.rows[0]?.do_not_contact) {
        status = "suppressed_do_not_contact"; leadId = null;
      } else {
        if (!customerId) {
          const customer = await client.query<{ id: string }>(`insert into customers(full_name,primary_phone,normalized_phone,email,city) values($1,$2,$3,$4,$5) returning id`,
            [mapped.fullName, mapped.phone, phone, null, mapped.city || null]);
          customerId = customer.rows[0].id;
        }
        status = "created_opportunity";
        const lead = await client.query<{ id: string }>(`insert into leads(customer_id,stage_id,source_id,priority,enquiry_summary,status) select $1,s.id,src.id,$2,$3,'unclaimed' from pipeline_stages s,lead_sources src where s.name='New Enquiry' and src.name='Jotform' returning id`,
          [customerId, mapped.priority, mapped.summary]);
        leadId = lead.rows[0].id;
        await client.query(`insert into audit_events(action,entity_type,entity_id,metadata) values('jotform.enquiry_received','lead',$1,$2)`,
          [leadId, JSON.stringify({ submissionId: submission.id, priority: mapped.priority })]);
      }

      await client.query(`insert into jotform_submissions(jotform_submission_id,submitted_at,payload,status,unmapped_fields,customer_id,lead_id) values($1,$2,$3,$4,$5,$6,$7) on conflict do nothing`,
        [submission.id, submission.created_at, JSON.stringify(submission), status, mapped.unmapped, customerId ?? null, leadId]);
      return status;
    });
    if (outcome === "created_opportunity") created++;
  }

  await query(`insert into jotform_sync_state(form_id,last_synced_submitted_at,last_synced_at,last_synced_by,last_run_created,last_run_skipped,last_run_held,last_run_error)
    values($1,$2,now(),$3,$4,$5,$6,null)
    on conflict(form_id) do update set last_synced_submitted_at=greatest(jotform_sync_state.last_synced_submitted_at,excluded.last_synced_submitted_at),
      last_synced_at=excluded.last_synced_at, last_synced_by=excluded.last_synced_by, last_run_created=excluded.last_run_created,
      last_run_skipped=excluded.last_run_skipped, last_run_held=excluded.last_run_held, last_run_error=null`,
    [formId, latestSubmittedAt, actor.id, created, skipped, held]);

  return { data: { fetched: submissions.length, created, skipped, held } };
});

app.get("/api/v1/intake/jotform/held", async (request, reply) => {
  if (!requireAdmin(request, reply)) return;
  const result = await query(`select id, jotform_submission_id as "submissionId", submitted_at as "submittedAt", payload, unmapped_fields as "unmappedFields" from jotform_submissions where status='held_for_review' order by submitted_at desc limit 100`);
  return { data: result.rows };
});
...
app.get("/api/v1/intake/jotform/status", async (request, reply) => {
  ...
  const result = await query("select * from jotform_sync_state where form_id=$1", [formId]);
  return { data: result.rows[0] ?? null, configured: Boolean(process.env.JOTFORM_API_KEY && formId) };
});
