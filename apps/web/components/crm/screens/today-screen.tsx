import { CalendarClock, Check, ChevronRight, CircleAlert, MessageCircle, Phone } from "lucide-react";
import type { Activity, Lead } from "@/lib/demo-data";
import type { CRMView } from "../types";
import { minutesUntil, Priority, telHref, time, whatsappHref } from "../shared/lead-format";
...
export function TodayScreen({ activities, unclaimed, noNext, onOpen, onComplete, onNavigate }: {
  ...
  return <div className="today-layout"><section className="today-main">
    {next && <div className="next-action"><div><span className="eyebrow light">Next activity</span><h2>{next.title}</h2><p>{next.customerName} · {next.phone}</p></div><div className="next-time"><strong>{time(next.scheduledStart)}</strong><span suppressHydrationWarning>{minutesUntil(next.scheduledStart)} min</span></div><div className="next-buttons"><a className="inverted" href={telHref(next.phone)}><Phone size={16} />Call</a><a className="ghost-light" href={whatsappHref(next.phone)} target="_blank" rel="noreferrer"><MessageCircle size={16} />WhatsApp</a><button className="ghost-light" onClick={() => onOpen(next.leadId)}>Open customer</button><button className="ghost-light" onClick={() => onComplete(next.id)}><Check size={16} />Complete</button></div></div>}
    ...
    <div className="activity-list">{active.map((activity) => <div className={`activity-row ${activity.status}`} key={activity.id}><time>{time(activity.scheduledStart)}</time><div className="activity-line"><span className="activity-icon">{activity.type === "Call" ? <Phone size={16} /> : <CalendarClock size={16} />}</span><div><strong>{activity.title}</strong><p>{activity.customerName} · <a className="contact-link" href={telHref(activity.phone)} onClick={(event) => event.stopPropagation()}>{activity.phone}</a>{activity.note ? ` · ${activity.note}` : ""}</p></div></div>{activity.status === "overdue" ? <span className="danger-label"><CircleAlert size={14} />Overdue</span> : <span className="quiet-label">{activity.type}</span>}<div className="row-actions"><a className="contact-link" href={whatsappHref(activity.phone)} target="_blank" rel="noreferrer" aria-label={`WhatsApp ${activity.customerName}`}><MessageCircle size={16} /></a><button onClick={() => onOpen(activity.leadId)}>Open</button><button className="check-button" aria-label="Complete" onClick={() => onComplete(activity.id)}><Check size={17} /></button></div></div>)}</div>
