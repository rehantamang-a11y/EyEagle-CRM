import { ArrowLeft, CalendarClock, MessageCircle, Phone, X } from "lucide-react";
import type { Activity, Lead } from "@/lib/demo-data";
import { day, initials, Priority, Stage, telHref, time, whatsappHref } from "../shared/lead-format";
...
export function LeadDrawer({ lead, activities, onClose, onClaim, onSchedule }: { lead: Lead; activities: Activity[]; onClose: () => void; onClaim: () => void; onSchedule: () => void }) {
  return <div className="overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="drawer">
    ...
    <div className="drawer-body">
      <div className="profile-title"><div className="large-initials">{initials(lead.customerName)}</div><div><h2>{lead.customerName}</h2><p><a className="contact-link" href={telHref(lead.phone)}>{lead.phone}</a> · {lead.city}</p></div></div>
      <div className="profile-state"><Stage name={lead.stage} /><span>{lead.ownerName || "Unclaimed"}</span><Priority priority={lead.priority} /></div>
      {lead.status === "unclaimed" ? <button className="primary full" onClick={onClaim}>Pick up this lead</button> : <div className="profile-actions"><button className="primary" onClick={onSchedule}><CalendarClock size={16} />Schedule follow-up</button><a className="secondary" href={telHref(lead.phone)}><Phone size={16} />Call</a><a className="secondary" href={whatsappHref(lead.phone)} target="_blank" rel="noreferrer"><MessageCircle size={16} />WhatsApp</a></div>}
